...
class ShareeToolbar : Toolbar {
    ...
    private fun initialize(context: Context) {
        ...
        inflater.inflate(R.layout.sharee_toolbar, this, true)
    }

    private fun initAttrs(attrs: AttributeSet) {
        val toolbarTypedArray =
            context.obtainStyledAttributes(attrs, R.styleable.ShareeToolbar)

        val titleContentDescription =
            toolbarTypedArray.getString(R.styleable.ShareeToolbar_titleContentDescription)
    ...
    private fun addAction(action: AbstractAction) {
        val menuItem = menu.add(action.titleResId)
            ...
            .setOnMenuItemClickListener({ menuItem1 -> callListener(action) })
        menuDrawables!!.add(menuItem.getIcon())
    }
